# Educational data

Datasets here are small, synthetic, and created solely for education. Chapter 1's
`harbor_incident_telemetry.csv` describes fictional operational measurements; it
contains no member or transaction records. Do not add real member, transaction,
credential, vendor, or proprietary banking data.

Chapters 2 and 3 use `harbor_request_outcomes.csv`, which contains 30
deterministic, fictional vendor-request observations and binary teaching labels.
It makes feature/target separation and a small train/test pipeline executable; it
is not statistically realistic and does not represent a real banking distribution.

Chapter 4 uses `harbor_normal_telemetry.csv`, a committed baseline of 200
deterministically generated ordinary-operation observations, and
`harbor_anomaly_scenarios.csv`, six named evaluation observations kept out of
training. The scenarios cover normal behavior, vendor slowdown, queue pressure,
broad pressure, and a subtle multivariate combination. The fixtures are stable for
the executable laboratory; they are not estimates of real production distributions
or Harbor's true incident rate.

Chapter 5 uses `harbor_incident_classes.csv`, a deterministic, balanced fixture of
300 labeled observations (60 per teaching class). Its overlapping synthetic
patterns support a multi-class classification exercise. The balance and values do
not estimate real incident frequency or describe a real production taxonomy.

Chapter 7 uses `harbor_integration_requests.csv`, 600 deterministic request-level
observations generated by `scripts/generate_integration_requests.py` with fixed
seed 707. Its overlapping binary outcomes combine fictional recent vendor metrics,
queue and retry pressure, request size, and modest endpoint interactions with
noise. No vendor is a deterministic label. The rules are educational and do not
represent real integration reliability, members, financial activity, or production
data.

Chapters 8–11 use `harbor_member_events.csv`, 9,808 events across 1,290 fictional sessions,
generated by `scripts/generate_member_events.py` with fixed seed 808. Its minimal
schema contains timestamps, pseudonymous session IDs, event names, web/mobile
channels, coarse journey names, educational landing sources, and coarse device
categories. It contains no direct member identifier,
financial value, credential, or authentication data. Generation patterns support
event grouping and descriptive analysis. Three hundred additional transfer sessions
support Chapter 9's partial-journey classification with noisy, overlapping synthetic
relationships and both outcomes. Chapter 10 adds 500 product-information sessions
with both application-start outcomes. Its overlapping, noisy relationships support
conversion prediction but do not estimate real campaign, device, channel, or member
behavior. Chapter 11 adds 400 varied session-pattern examples for unsupervised
clustering; its generation tendencies are not stored as labels or model targets.

Chapter 12 uses `harbor_transaction_observations.csv`, 800 deterministic fictional
baseline observations generated with seed 1212. Its coarse amount bands, generic
transaction types, operational channels, and contextual measurements contain no
direct member identity or exact transaction amount. It has no anomaly target.
`harbor_transaction_scenarios.csv` holds five manually designed evaluation probes
kept out of training. These fixtures demonstrate mixed-feature preprocessing and
relative unusualness; they are not realistic transaction distributions, fraud
labels, evidence of wrongdoing, or estimates of any real rate.


Chapter 13 uses `harbor_review_routing.csv`, 1,000 deterministic fictional labeled
observations generated by `scripts/generate_review_routing.py` with seed 1313. Its
270 positive values mean only that the synthetic historical process routed the
observation for additional operational review. Overlapping relationships and noise
prevent any one signal from determining the label. The fixture contains no direct
identity or exact amount and is not fraud, wrongdoing, credit, eligibility, member-
quality, or real operational data.


Chapter 14 uses `harbor_system_demand.csv`, 5,760 fictional observations at fixed
five-minute intervals across 20 synthetic days. It is generated by
`scripts/generate_system_demand.py` with seed 1414. Smooth daily and weekly teaching
patterns, bounded event effects, and controlled noise support chronological regression.
It contains no member records or real production telemetry and is not representative of
actual credit-union demand, reliability, latency, error, or capacity distributions.

Chapter 15 uses `harbor_query_performance.csv`, 1,800 strictly chronological,
fictional observations generated by `scripts/generate_query_performance.py` with
seed 1515. Simplified query families, estimated row bands, query-shape flags, and
current system-state values produce a noisy, nonlinear duration target for teaching
pre-execution regression. It contains no SQL, real schema, member records, or real
production measurements and does not represent credit-union database performance.

Chapter 27 reuses `harbor_capstone_incident.csv`, `harbor_capstone_traces.csv`, and
`harbor_request_outcomes.csv` through separate typed source views. It writes the
derived training CSV and SHA-256 metadata only under gitignored
`artifacts/capstone-dataset/`; no derived dataset is committed. Database latency in
this compact teaching build is an explicitly named deterministic proxy rather than
a real instrument. Trace completion evidence and later outcomes are never model
features. All records remain fictional and privacy-minimized.
